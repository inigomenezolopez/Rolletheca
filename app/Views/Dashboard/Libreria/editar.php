...
<div class="container mt-4">
        ...
        <div class="col-md-8 offset-md-2">
            <div class="card shadow" style="border-color: #cbdbe5;">
                ...
                <div class="card-body" style="color: #1b2530;">
                    <form action="/libreria/actualizar/<?= esc($libro->id) ?>" method="post" enctype="multipart/form-data">
                        <div class="form-group">
                            <label for="titulo" class="form-label">Título:</label>
                            <input type="text" name="titulo" id="titulo" class="form-control" placeholder="Título" value="<?= old('titulo', esc($libro->titulo)) ?>">
                        </div>
                        <div class="form-group">
                            ...
                            <select name="id_categoria" id="id_categoria" class="form-control">
                                ...
                                <?php  foreach ($categorias as $categoriaItem) : ?>
                                    <option <?= $categoriaItem->id == old('id_categoria', esc($libro->id_categoria)) ? 'selected' : '' ?> value="<?= $categoriaItem->id ?>"><?= $categoriaItem->nombre ?></option>
                                <?php endforeach ?>
                            </select>
                        </div>
                        <div class="form-group">
    ...
    <?php foreach ($etiquetasDeLaCategoria as $etiquetaItem): ?>
        <option value="<?= $etiquetaItem->id ?>" <?= in_array($etiquetaItem->id, $etiquetasAsignadasIds) ? 'selected' : '' ?>>
    ...
    <?php endforeach; ?>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="descripcion" id="descripcion" class="form-control" rows="4"><?= old('descripcion', esc($libro->descripcion)) ?></textarea>
                        </div>
                        <div class="mb-3">
    <label for="imagen" class="form-label">Imagen</label>
    <!-- Muestra la imagen actual -->
    <?php if (isset($libro->ruta_archivo) && file_exists(FCPATH . 'images/libreria/' . $libro->ruta_archivo)): ?>
        <img src="<?= base_url('images/libreria/' . esc($libro->ruta_archivo)) ?>" alt="Imagen actual" class="img-thumbnail"> 
    <?php endif; ?>
    ...
    <input type="hidden" name="ruta_archivo_actual" value="<?= esc($libro->ruta_archivo ?? '') ?>">
</div>
